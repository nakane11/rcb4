...
class ESP32Interface(object):
    ...
    def open(self, port='tcp', tcp_portnum="11411"):
        #Opens a connection to device via rosserial.
        self.serial = subprocess.Popen(
            [f'/opt/ros/{os.environ["ROS_DISTRO"]}/bin/rosrun',
             'rosserial_python', 'serial_node.py', port, tcp_portnum])
        while True:
            log = rospy.wait_for_message('/rosout', Log)
            if log.name == "/serial_node":
                if "Setup" in log.msg:
